...
data("Andropogon_shape", package = "bayesLopod")
ld_Shape = shapeLopodData(Shapefile = Andropogon_shape, fieldN = "sampEffort", fieldY = "detections",  Adjacency = T, keepFields = F)
mLopodShape = modelLopod(LopodData = ld_Shape, varP = T, q = NULL, pmin = 0, CAR = T, nChains = 4,warmup = 400,sampling = 100,nCores =4)
...
data("mLopodShape", package = "bayesLopod")
psyShape = lopodShape(mLopodShape, "psi_i", extrapolate = F,  quant = 0.5)
sp::spplot(psyShape, zcol = "psi_i")


data("mLopodShape", package = "bayesLopod")
psyShape = lopodShape(mLopodShape, "psi_i", extrapolate = T, quant = 0.05)
sp::spplot(psyShape, zcol = "psi_i")
