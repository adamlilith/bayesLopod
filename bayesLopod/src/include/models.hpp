...
public:
    ...
    T__ log_prob(vector<T__>& params_r__,
        ...
        try {
            ...
            lp_accum__.add(normal_log(p,1,0.25));
...
public:
    ...
    void transform_inits(const stan::io::var_context& context__,
        ...
        // generate_declaration pmax
        double pmax(0);
        pmax = vals_r__[pos__++];
        try {
            writer__.scalar_lub_unconstrain(stan::math::fmax(minP,q),1,pmax);
        } catch (const std::exception& e) { 
        ...
        // generate_declaration pmin
        double pmin(0);
        pmin = vals_r__[pos__++];
        try {
            writer__.scalar_lub_unconstrain(stan::math::fmax(minP,q),1,pmin);
        } catch (const std::exception& e) { 
    ...
    T__ log_prob(vector<T__>& params_r__,
        ...
        if (jacobian__)
            pmax = in__.scalar_lub_constrain(stan::math::fmax(minP,q),1,lp__);
        else
            pmax = in__.scalar_lub_constrain(stan::math::fmax(minP,q),1);

        T__ pmin;
        (void) pmin;  // dummy to suppress unused var warning
        if (jacobian__)
            pmin = in__.scalar_lub_constrain(stan::math::fmax(minP,q),1,lp__);
        else
            pmin = in__.scalar_lub_constrain(stan::math::fmax(minP,q),1);
    ...
    void write_array(RNG& base_rng__,
        ...
        double pmax = in__.scalar_lub_constrain(stan::math::fmax(minP,q),1);
        double pmin = in__.scalar_lub_constrain(stan::math::fmax(minP,q),1);
        for (int k_0__ = 0; k_0__ < nSampledCells; ++k_0__) {
            vars__.push_back(psy_Sampled[k_0__]);
        }
        for (int k_0__ = 0; k_0__ < nSampledCells; ++k_0__) {
...
public:
    ...
    void transform_inits(const stan::io::var_context& context__,
        ...
        pmax = vals_r__[pos__++];
        try {
            writer__.scalar_lub_unconstrain(minP,1,pmax);
        } catch (const std::exception& e) { 
            throw std::runtime_error(std::string("Error transforming variable pmax: ") + e.what());
        }
        ...
        pmin = vals_r__[pos__++];
        try {
            writer__.scalar_lub_unconstrain(minP,1,pmin);
        } catch (const std::exception& e) { 
            throw std::runtime_error(std::string("Error transforming variable pmin: ") + e.what());
        }
        ...
        q = vals_r__[pos__++];
        try {
            writer__.scalar_lub_unconstrain(0,1,q);
        } catch (const std::exception& e) { 
    ...
    T__ log_prob(vector<T__>& params_r__,
        ...
        (void) pmax;  // dummy to suppress unused var warning
        if (jacobian__)
            pmax = in__.scalar_lub_constrain(minP,1,lp__);
        else
            pmax = in__.scalar_lub_constrain(minP,1);

        T__ pmin;
        (void) pmin;  // dummy to suppress unused var warning
        if (jacobian__)
            pmin = in__.scalar_lub_constrain(minP,1,lp__);
        else
            pmin = in__.scalar_lub_constrain(minP,1);

        T__ q;
        (void) q;  // dummy to suppress unused var warning
        if (jacobian__)
            q = in__.scalar_lub_constrain(0,1,lp__);
        else
            q = in__.scalar_lub_constrain(0,1);
    ...
    void write_array(RNG& base_rng__,
        ...
        vector_d psy_Sampled = in__.vector_lub_constrain(0,1,nSampledCells);
        vector_d p_raw = in__.vector_lub_constrain(0,1,nSampledCells);
        double pmax = in__.scalar_lub_constrain(minP,1);
        double pmin = in__.scalar_lub_constrain(minP,1);
        double q = in__.scalar_lub_constrain(0,1);
        for (int k_0__ = 0; k_0__ < nSampledCells; ++k_0__) {
            vars__.push_back(psy_Sampled[k_0__]);
        }
        for (int k_0__ = 0; k_0__ < nSampledCells; ++k_0__) {
...
public:
    ...
    void transform_inits(const stan::io::var_context& context__,
        ...
        p = vals_r__[pos__++];
        try {
            writer__.scalar_lub_unconstrain(minP,1,p);
        } catch (const std::exception& e) { 
            throw std::runtime_error(std::string("Error transforming variable p: ") + e.what());
        }
    ...
    T__ log_prob(vector<T__>& params_r__,
        ...
        (void) p;  // dummy to suppress unused var warning
        if (jacobian__)
            p = in__.scalar_lub_constrain(minP,1,lp__);
        else
            p = in__.scalar_lub_constrain(minP,1);
    ...
    void write_array(RNG& base_rng__,
        ...
        vector_d psy_Sampled = in__.vector_lub_constrain(0,1,nSampledCells);
        double q = in__.scalar_lub_constrain(0,1);
        double p = in__.scalar_lub_constrain(minP,1);
        for (int k_0__ = 0; k_0__ < nSampledCells; ++k_0__) {
            vars__.push_back(psy_Sampled[k_0__]);
        }
        vars__.push_back(q);
